...
//  }
  ...
  static Widget getPrice(double myPrice, BuildContext context, {TextStyle style}) {
    ...
    try {
      if (myPrice == 0) {
        return Text('-', style: style ?? Theme.of(context).textTheme.subtitle);
      }
      return RichText(
        softWrap: false,
        ...
        text: setting.value?.currencyRight != null && setting.value?.currencyRight == false
            ? TextSpan(
                ...
                children: <TextSpan>[
                  TextSpan(text: myPrice.toStringAsFixed(2) ?? '', style: style ?? Theme.of(context).textTheme.subtitle),
                ],
              )
            : TextSpan(
                ...
                children: <TextSpan>[
                  TextSpan(
                      text: setting.value?.defaultCurrency,
                      style: TextStyle(
                          fontWeight: FontWeight.w400, fontSize: style != null ? style.fontSize - 4 : Theme.of(context).textTheme.subtitle.fontSize - 4)),
                ],
              ),
      );
    } catch (e) {
  ...
  static Html applyHtml(context, String html, {TextStyle style}) {
    return Html(
      blockSpacing: 0,
      data: html ?? '',
      defaultTextStyle: style ?? Theme.of(context).textTheme.body1.merge(TextStyle(fontSize: 14)),
      useRichText: false,
